...
String servo1;     //contiendra 1 à 3 characteres provenant du buffer Serial
...
int gazAlarme = 0;         //indicateur que l'alarme de gaz est déjà activée
...
void traiter_Alarmes() {
  if (gazFlag) {
    ...
    //partir la fan vers l'avant au max
    //ouvrir la fenêtre
    //afficher le bon code d'alarme sur l'écran LCD
    if (!gazAlarme) {
      //mettre l'alarme sur la pile d'alarme
      gazAlarme = true;
    }
    //penser à désactiver l'alarme?????? ( pas obligé pour le 12 sept 2022)
  }
}
...
void pwm_control() {
  switch (val) {
    case 't':  //if val is 't'，program will circulate
      ...
      delay(300); //coudonc, il faut tester si c'est nécessaire
      break;   //exit loop
